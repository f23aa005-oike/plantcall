...
- `setup.sh`: 自動セットアップ Bash スクリプト (推奨)

---

## 1.5. 自動セットアップスクリプトの使用 (推奨)

本システムでは、依存ライブラリのインストール、udevルールの設定、systemdサービスの設定を全自動で行う `setup.sh` を同梱しています。

### 実行手順:
1. `localmonitor` ディレクトリに移動します。
2. 以下のコマンドを実行します。
   ```bash
   chmod +x setup.sh
   ./setup.sh
   ```
   *(※内部で `sudo` を使用した udev ルールや systemd サービスファイルの配置を行うため、実行中に管理者パスワードの入力が求められます。)*

これにより、後述の手動手順（セクション2〜4）がすべて自動的に完了します。自動セットアップに失敗した場合や、手動でカスタマイズを行いたい場合は、以下の手順を参考にしてください。
